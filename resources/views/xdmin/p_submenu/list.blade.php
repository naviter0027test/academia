...
@section('js')
    ...
    <script>
        ...
        var columnDefs = [
            ...
            {headerName: "中文名稱", field: "title"},
            {headerName: "權重", field: "weights"},
        ];
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            $.get('/xdmin/psubmenu/json',
                function (data) {
                    gridOptions.api.setRowData(data);
                });
        });
        function addNewData() {
            document.location.href="/xdmin/psubmenu/edit/-1";
        }
        function deleteData() {
            if(gridOptions.api.getSelectedRows().length  > 0) {
                var row = gridOptions.api.getSelectedRows();
                if(confirm("確定要刪除該筆資料?")){
                    $.post('/xdmin/psubmenu/del',{_token:_token,id:row[0].id},
                        function (data) {
                            $.get('/xdmin/psubmenu/json',
                                function (data) {
                                    gridOptions.api.setRowData(data);
        ...
        function editData() {
            if(gridOptions.api.getSelectedRows().length  > 0) {
                var row = gridOptions.api.getSelectedRows();
                document.location.href="/xdmin/psubmenu/edit/"+row[0].id;
            } else {
                alert("請先選擇相對作業內容");
            }
        }

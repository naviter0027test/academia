        ...
        <div class="col-md-12">
            ...
            <div class="portlet box green">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-gift"></i>般若教育({{$type}})
                    </div>
                    <div class="tools">
...
@section('js')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            $.get('/xdmin/list2/{{$type}}/json',
                function (data) {
                    gridOptions.api.setRowData(data);
                });
        });
        function addNewData() {
            document.location.href="/xdmin/list2/{{$type}}/edit/-1";
        }
        function deleteData() {
            if(gridOptions.api.getSelectedRows().length  > 0) {
                var row = gridOptions.api.getSelectedRows();
                if(confirm("確定要刪除該筆資料?")){
                    $.post('/xdmin/list2/del',{_token:_token,id:row[0].id},
                        function (data) {
                            $.get('/xdmin/list2/{{$type}}/json',
                                function (data) {
                                    gridOptions.api.setRowData(data);
        ...
        function editData() {
            if(gridOptions.api.getSelectedRows().length  > 0) {
                var row = gridOptions.api.getSelectedRows();
                document.location.href="/xdmin/list2/{{$type}}/edit/"+row[0].id;
            } else {
                alert("請先選擇相對作業內容");
            }
        }
